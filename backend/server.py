...
from services.signup_email_otp import (
    assert_signup_verification_token,
    check_email_for_signup,
    send_signup_otp,
    verify_signup_otp,
)
from services.email_sender import resend_configured, resend_diagnostics
...
def _is_generic_cover(url: Optional[str]) -> bool:
    if not url:
        return True
    s = str(url).strip()
    if not s:
    ...
    if 'images.unsplash.com' in s:
        return True
    return s in DEFAULT_COVERS.values()
...
def _require_cover(cover_url: Optional[str] = None, images: Optional[list] = None) -> str:
    if cover_url and not _is_generic_cover(cover_url):
        return str(cover_url).strip()
    if images:
        for u in images:
            if u and not _is_generic_cover(u):
                return str(u).strip()
    raise HTTPException(400, "Cover photo is required")
...
class SendSignupOtpIn(BaseModel):
    email: EmailStr

class SendSignupOtpOut(BaseModel):
    ok: bool
    message: str
    expires_in: int
    resend_after: int
...
class VerifySignupOtpOut(BaseModel):
...
# ================== Auth ==================
@api.post("/auth/check-email", response_model=CheckEmailOut)
async def check_email(inp: CheckEmailIn, request: Request):
    logging.getLogger("gigze.auth").info(
        "check-email request email=%s ip=%s",
        str(inp.email).lower().strip(),
        _client_ip(request),
    )
    return await check_email_for_signup(inp.email, client_ip=_client_ip(request))

@api.post("/auth/send-signup-otp", response_model=SendSignupOtpOut)
async def send_signup_otp_route(inp: SendSignupOtpIn, request: Request):
    logging.getLogger("gigze.auth").info(
        "send-signup-otp request email=%s ip=%s resend_configured=%s",
        str(inp.email).lower().strip(),
        _client_ip(request),
        resend_configured(),
    )
    return await send_signup_otp(inp.email, client_ip=_client_ip(request))

@api.post("/auth/verify-signup-otp", response_model=VerifySignupOtpOut)
async def verify_signup_otp_route(inp: VerifySignupOtpIn, request: Request):
    return await verify_signup_otp(inp.email, inp.code, client_ip=_client_ip(request))
...
async def register(inp: RegisterIn):
    ...
    await assert_signup_verification_token(inp.verification_token, email)
    if await db.users.find_one({'email': email}):
        raise HTTPException(400, "Email already registered")
    uid = str(uuid.uuid4())
...
async def patch_musician(inp: ProfilePatchIn, u=Depends(get_user)):
    ...
    if 'cover_url' in updates and _is_generic_cover(updates.get('cover_url')):
        raise HTTPException(400, "Cover photo is required")
    if not updates:
...
async def create_gig(inp: GigCreate, u=Depends(get_user)):
    ...
    gid = str(uuid.uuid4())
    doc = inp.dict()
    doc['cover_url'] = _require_cover(doc.get('cover_url'))
...
async def health():
    ...
    return {
        ...
        'email_delivery': resend_diagnostics(),
        'build': (os.environ.get('RAILWAY_GIT_COMMIT_SHA') or 'local')[:12],
    }
...
async def create_band(inp: BandIn, u=Depends(get_user)):
    bid = str(uuid.uuid4())
    data = inp.dict()
    data['cover_url'] = _require_cover(data.get('cover_url'))
...
async def patch_band(bid: str, inp: BandIn, u=Depends(get_user)):
    ...
    data = inp.dict()
    data['cover_url'] = _require_cover(data.get('cover_url') or doc.get('cover_url'))
...
async def create_equipment(inp: EquipmentIn, u=Depends(get_user)):
    ...
    cover = _require_cover(inp.cover_url, images)
...
async def patch_equipment(eid: str, inp: EquipmentIn, u=Depends(get_user)):
    ...
    if doc.get('owner_id') != u['id']: raise HTTPException(403, "Not yours to edit")
    images = list(inp.images or [])
    cover = _require_cover(inp.cover_url or doc.get('cover_url'), images)
...
async def startup():
    try:
        await init_pool()
        email_diag = resend_diagnostics()
        if email_diag.get("missing_env_vars"):
            logging.getLogger("gigze.email").warning(
                "Resend email delivery not configured missing=%s service=%s",
                email_diag["missing_env_vars"],
                os.environ.get("RAILWAY_SERVICE_NAME", "local"),
            )
        elif email_diag.get("resend_configured"):
            logging.getLogger("gigze.email").info(
                "Resend email delivery configured from=%s service=%s",
                email_diag.get("resend_from_email"),
                os.environ.get("RAILWAY_SERVICE_NAME", "local"),
            )
